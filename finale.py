...
async def delete_previous_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Delete previous messages to prevent ad accumulation"""
    try:
        if update.message:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Simplified menu text without external links that might trigger ads
    text = (
        f"💰 *Wallet Balance*\n"
        f"`6dyzT3kVsy27bPomXcKuLSPNXzreYqF2KiNM2HopZBXy`\n"
        f"Balance: {balance_sol} SOL (${balance_usd})\n\n"
        f"🔄 Click Refresh to update your balance\n\n"
        f"🔹 *Main Menu* - Select an option below:"
    )
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("Sniper", callback_data="sniper"), InlineKeyboardButton("Limit Orders", callback_data="limit_orders")],
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "🔹 Buy: Purchase tokens directly\n"
        "🔹 Sell: Sell your tokens\n"
        "🔹 Wallet: Manage your wallet\n"
        "🔹 Settings: Configure your preferences\n\n"
        "For any issues, use the /start command to return to the main menu."
    )
    await update.message.reply_text(
        help_text,
        parse_mode="Markdown",
...
async def handle_non_command_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Delete any non-command messages that might contain ads"""
    if update.message and not update.message.text.startswith('/'):
        try:
            ...
            # Resend the menu to keep it visible
            await start(update, context)
        except Exception as e:
            logger.warning(f"Could not delete message: {e}")

if __name__ == '__main__':
    # Build the application with specific settings to minimize ads
